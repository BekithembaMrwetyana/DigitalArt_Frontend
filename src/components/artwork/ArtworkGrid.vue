...
export default {
  ...
  setup(props, { emit }) {
    ...
    const displayedArtworks = computed(() => {
      if (props.infinite) {
        return props.artworks.slice(0, currentPage.value * props.itemsPerPage)
      }
    ...
    const hasMore = computed(() => {
      return props.infinite && 
             displayedArtworks.value.length < props.artworks.length
    })
    ...
    // Reset pagination when artworks change
